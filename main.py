...
def main():
    start_time = time.time()  # Додано для логування часу роботи
    time.sleep(0.1)
    ...
    try:
        ...
        end_time = time.time()  # Додано для обчислення часу роботи сервера
        elapsed_time = end_time - start_time
        print(f"Сервер зупинено. Час роботи: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time))}.")
        logging.info(f"[SERVER] Сервер зупинено. Час роботи: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time))}.")
